...
const createPbkdf = (password) => {
  // pbkdf2함수(비밀번호, 솔트, 반복횟수, 키의길이, 알고리즘) 으로 생성되고
  // 반환되는 값은 Buffer 값
...
/** crypto - 양방향 */
// 암호화
const algorithm = "aes-256-cbc"; // 256 비트 키를 사용, 블록 크기는 128 비트
const key = crypto.randomBytes(32); // 1 byte = 8 bit, 32 byte = 256 bit
const iv = crypto.randomBytes(16); // 초기화 벡터, 데이터 블록을 암호화할때 보안성 높이기 위해 사용한다.
